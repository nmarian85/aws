...
def multi_part_upload_with_s3(fmd5, f, s3, bucket, s3_ddir, logger):
        mpu = create_multipart_upload(Bucket=self.bucket, Key=self.key)
        mpu_id = mpu["UploadId"]
        ...
        try:
                s3.meta.client.upload_file(Filename=f, Bucket=bucket, Key=s3_path, Config=config, ExtraArgs={'ContentMD5': fmd5})
